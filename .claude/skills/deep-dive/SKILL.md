...
2. **GitHub repo's `/docs/` tree** (markdown / mdx):
   - For **file content**, prefer the raw host: `WebFetch https://raw.githubusercontent.com/<owner>/<repo>/main/<path>` — the rendered `github.com/<owner>/<repo>/blob/main/<path>` page often returns just the GitHub UI shell with the file's body buried in JSON, especially for large files (~observed on pydantic's `HISTORY.md`). Raw is the file, nothing else.
   - For **directory listings or file existence checks**, the `blob`/`tree` rendered pages are fine.
   - Top-level README: `WebFetch https://raw.githubusercontent.com/<owner>/<repo>/main/README.md`.
   - Deep doc pages: `WebFetch https://raw.githubusercontent.com/<owner>/<repo>/main/docs/<area>.mdx`. Common areas: `docs/getting-started/`, `docs/deployment/`, `docs/servers/`, `docs/integrations/`, `docs/patterns/`. Don't guess all paths — `WebSearch "<tool> <feature> site:github.com"` first when the structure is unknown.
   - If a path 404s, recover via `WebSearch "<tool> <feature>"` and follow the first GitHub-hosted hit (then switch to its `raw.githubusercontent.com` equivalent).
3. **GitHub releases / CHANGELOG**:
   - `WebFetch https://raw.githubusercontent.com/<owner>/<repo>/main/CHANGELOG.md` (or `HISTORY.md`, `RELEASES.md` — check the repo). Use the raw host for the same reason as above.
   - GitHub-hosted `/releases` page is fine for the rendered timeline if you just need version numbers + dates.
...
- Show the resulting diff (`git diff -- slots/<category>/<tool>/README.md`) so the user sees what landed.
- **Commit and push** in a single dedicated commit (the repo's stop-hook requires a clean working tree):
  - Stage **only** the slot's README — never bundle the deep-dive with unrelated changes.
  - Commit message format:
    ```
    add deep-dive section to <tool> slot (YYYY-MM-DD)

    Generated by the deep-dive skill against <tool> v<X.Y.Z> in Mode <A|B|C>.
    <1–3 lines on the top concrete finding(s) — e.g. stale pin, anti-pattern
    leak, missing feature — so the commit log doubles as a changelog.>

    Section is additive at the bottom of the slot README; curated body
    above is unchanged.
    ```
  - Push to the current branch with `git push -u origin <branch>`.
- If the user later wants to revert, they delete the section in a follow-up commit — never `--amend` or force-push.
